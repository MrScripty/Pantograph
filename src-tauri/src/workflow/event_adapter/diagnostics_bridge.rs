...
use super::translation::{translate_node_event, translated_workflow_run_id};
...
pub(super) fn translate_node_event_with_diagnostics(
    ...
    let source_workflow_run_id = translated_workflow_run_id(&tauri_event).to_string();
    let diagnostics_snapshot = trace_event
        ...
        .with_source_workflow_run_id(Some(source_workflow_run_id.clone()));
    let diagnostics_event =
        TauriWorkflowEvent::diagnostics_snapshot(source_workflow_run_id, diagnostics_snapshot);
