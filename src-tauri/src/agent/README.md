...
for RIG tool-calling, and RAG uses an explicit host RAG embedding adapter for
documentation indexing/search. These adapters are not workflow inference
backends and must not be reused by graph execution or model-runtime selection
code.
...
- Host-agent model clients are scoped to desktop assistant tool-calling and RAG
  only; workflow execution must use the canonical inference/workflow contracts.
