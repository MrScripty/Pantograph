...
impl Task for ImageInputTask {
    ...
    async fn run(&self, context: Context) -> graph_flow::Result<TaskResult> {
        ...
        let image_key = ContextKeys::input(&self.task_id, "image_base64");
        ...
        // Get optional bounds
        let bounds_key = ContextKeys::input(&self.task_id, "bounds");
        let bounds: Option<ImageBounds> = context.get(&bounds_key).await;

        // Store outputs in context
        let output_image_key = ContextKeys::output(&self.task_id, "image");
        context.set(&output_image_key, image_base64.clone()).await;

        let output_bounds_key = ContextKeys::output(&self.task_id, "bounds");
