            ...
            runtime_capabilities: vec![WorkflowRuntimeCapability {
                ...
                backend_capability_facts: Some(WorkflowBackendCapabilityFacts {
                    ...
                    model_sources: WorkflowBackendModelSourceCapabilityFacts {
                        artifact_kinds: vec![WorkflowModelArtifactKind::Gguf],
                        backend_hints: vec![WorkflowBackendHintLabel::LlamaCpp],
...
fn workflow_scheduler_timeline_query_contract_snapshot() {
    ...
    let expected_response = serde_json::json!({
        "events": [{
            ...
            "scheduler_attempt_id": null,
            "scheduler_attempt_execution_class": null,
            "scheduler_attempt_transition": null,
            "scheduler_attempt_started_at_ms": null,
            "scheduler_attempt_ended_at_ms": null,
            "scheduler_attempt_duration_ms": null,
            "scheduler_attempt_runtime_id": null,
            "scheduler_attempt_runtime_variant_id": null,
            "scheduler_attempt_backend_key": null,
            "scheduler_attempt_device_class": null,
            "scheduler_attempt_device_id": null,
            "scheduler_attempt_network_node_id": null,
            "scheduler_attempt_reservation_id": null,
...
fn workflow_run_list_query_contract_snapshot() {
    let request = pantograph_workflow_service::WorkflowRunListQueryRequest {
        ...
        selected_device_id: Some("local-gpu-0".to_string()),
        selected_network_node_id: Some("local-node".to_string()),
        client_id: Some("client-1".to_string()),
        client_session_id: Some("session-1".to_string()),
        bucket_id: Some("bucket-1".to_string()),
        accepted_at_from_ms: Some(900),
        accepted_at_to_ms: Some(1200),
    ...
    let response = pantograph_workflow_service::WorkflowRunListQueryResponse {
        runs: vec![RunListProjectionRecord {
            ...
            selected_runtime_variant_id: Some("llama_cpp.cuda".to_string()),
            selected_backend_key: Some("llama_cpp".to_string()),
            selected_model_id: Some("pumas://models/tiny-gguf".to_string()),
            selected_task_id: Some("text_generation".to_string()),
            selected_device_class: Some("cuda".to_string()),
            selected_device_id: Some("local-gpu-0".to_string()),
            selected_network_node_id: Some("local-node".to_string()),
            ...
            latest_error_severity: Some(DiagnosticErrorSeverity::Fatal),
            latest_error_phase: Some("runtime_model_load".to_string()),
            latest_error_code: Some("runtime_model_load_failed".to_string()),
            latest_error_message: Some("backend failed".to_string()),
            fatal_error_event_id: Some("error-event-1".to_string()),
            error_count: 1,
            warning_count: 0,
            last_event_seq: 15,
        ...
        facets: vec![
            ...
            RunListFacetRecord {
                facet_kind: RunListFacetKind::SelectedRuntime,
    ...
    let expected_request = serde_json::json!({
        ...
        "selected_runtime_id": "llama_cpp",
        "selected_runtime_variant_id": "llama_cpp.cuda",
        "selected_backend_key": "llama_cpp",
        "selected_device_class": "cuda",
        "selected_device_id": "local-gpu-0",
        "selected_network_node_id": "local-node",
        "client_id": "client-1",
        "client_session_id": "session-1",
        "bucket_id": "bucket-1",
        "accepted_at_from_ms": 900,
        "accepted_at_to_ms": 1200,
    ...
    let expected_response = serde_json::json!({
        "runs": [{
            ...
            "observed_peak_ram_bytes": null,
            "observed_peak_vram_bytes": null,
            "memory_failure_kind": null,
            "output_artifact_count": 1,
            ...
            "latest_error_message": "backend failed",
...
#[test]
fn workflow_run_detail_query_contract_snapshot() {
    let request = WorkflowRunDetailQueryRequest {
        workflow_run_id: "run-1".to_string(),
        projection_batch_size: Some(100),
    };
    let response = WorkflowRunDetailQueryResponse {
        run: Some(RunDetailProjectionRecord {
            workflow_run_id: "run-1".to_string().try_into().expect("run id"),
            workflow_id: "wf-1".to_string().try_into().expect("workflow id"),
            workflow_version_id: Some("version-1".to_string().try_into().expect("version id")),
            workflow_semantic_version: Some("1.2.3".to_string()),
            ...
            started_payload_json: Some(r#"{"queue_wait_ms":10}"#.to_string()),
            terminal_payload_json: Some(r#"{"status":"completed"}"#.to_string()),
            terminal_error: None,
            scheduler_queue_position: Some(1),
            scheduler_priority: Some(5),
            estimate_confidence: Some("low".to_string()),
        ...
        node_statuses: vec![NodeStatusProjectionRecord {
            workflow_run_id: "run-1".to_string().try_into().expect("run id"),
            workflow_id: "wf-1".to_string().try_into().expect("workflow id"),
            workflow_version_id: Some("version-1".to_string().try_into().expect("version id")),
            workflow_semantic_version: Some("1.2.3".to_string()),
    ...
    let request_value = serde_json::to_value(request).expect("serialize run detail request");
    let expected_request = serde_json::json!({
        "workflow_run_id": "run-1",
        "projection_batch_size": 100
    });
    ...
    let response_value = serde_json::to_value(response).expect("serialize run detail response");
    let expected_response = serde_json::json!({
        "run": {
            "workflow_run_id": "run-1",
            "workflow_id": "wf-1",
            ...
            "started_payload_json": "{\"queue_wait_ms\":10}",
            "terminal_payload_json": "{\"status\":\"completed\"}",
            "terminal_error": null,
            "scheduler_queue_position": 1,
            "scheduler_priority": 5,
            "estimate_confidence": "low",
...
fn workflow_io_artifact_query_contract_snapshot() {
    ...
    let response = WorkflowIoArtifactQueryResponse {
        artifacts: vec![IoArtifactProjectionRecord {
            ...
            model_id: Some("model-1".to_string()),
            model_version: Some("rev-1".to_string()),
            artifact_fact_id: "fact-1".to_string(),
            payload_artifact_id: "artifact-1".to_string(),
            artifact_id: "artifact-1".to_string(),
            artifact_role: "node_output".to_string(),
            logical_payload_lineage_id: Some("lineage-1".to_string()),
            producer_node_id: Some("node-1".to_string()),
            producer_port_id: Some("image".to_string()),
            consumer_node_id: None,
    ...
    let expected_response = serde_json::json!({
        "artifacts": [{
            ...
            "model_id": "model-1",
            "model_version": "rev-1",
            "artifact_fact_id": "fact-1",
            "payload_artifact_id": "artifact-1",
            "artifact_id": "artifact-1",
            "artifact_role": "node_output",
            "logical_payload_lineage_id": "lineage-1",
            "producer_node_id": "node-1",
            "producer_port_id": "image",
...
#[test]
fn workflow_admin_queue_push_front_contract_snapshot() {
    let request = WorkflowAdminQueuePushFrontRequest {
        workflow_run_id: "run-admin-3".to_string(),
    };
    let response = WorkflowAdminQueuePushFrontResponse {
        ok: true,
        session_id: "session-3".to_string(),
        priority: 23,
    };

    let request_value = serde_json::to_value(request).expect("serialize admin push-front request");
    let expected_request = serde_json::json!({
        "workflow_run_id": "run-admin-3"
    });
    assert_eq!(request_value, expected_request);

    let response_value =
        serde_json::to_value(response).expect("serialize admin push-front response");
    let expected_response = serde_json::json!({
        "ok": true,
