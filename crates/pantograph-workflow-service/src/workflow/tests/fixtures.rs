...
pub(super) struct MockWorkflowHost {
    pub(super) capabilities: WorkflowHostCapabilities,
    pub(super) omit_requested_target_output: bool,
    pub(super) emit_invalid_output_binding: bool,
    pub(super) technical_fit_decision: Option<WorkflowTechnicalFitDecision>,
    pub(super) recorded_run_options: Arc<Mutex<Vec<WorkflowRunOptions>>>,
}

impl MockWorkflowHost {
    pub(super) fn new(max_input_bindings: usize, max_value_bytes: usize) -> Self {
        Self {
    ...
    pub(super) fn with_missing_requested_output(
    ...
    pub(super) fn with_invalid_output_binding(
        max_input_bindings: usize,
        max_value_bytes: usize,
    ) -> Self {
        Self {
    ...
    pub(super) fn with_technical_fit_decision(
        max_input_bindings: usize,
        max_value_bytes: usize,
...
pub(super) struct InspectionHost {
    pub(super) calls: Arc<Mutex<Vec<(String, String)>>>,
    pub(super) state: Option<WorkflowGraphSessionStateView>,
}
...
pub(super) fn ready_runtime_capability() -> WorkflowRuntimeCapability {
...
pub(super) fn ready_pytorch_runtime_capability() -> WorkflowRuntimeCapability {
    WorkflowRuntimeCapability {
...
pub(super) struct TimeoutAwareHost {
...
#[derive(Clone)]
pub(super) struct BlockingRunHost {
    pub(super) capabilities: WorkflowHostCapabilities,
    pub(super) started_runs: Arc<AtomicUsize>,
    pub(super) first_run_started: Arc<Notify>,
    pub(super) release_first_run: Arc<Notify>,
}
...
impl BlockingRunHost {
    ...
    pub(super) async fn wait_for_first_run_started(&self) {
    ...
    pub(super) fn release_first_run(&self) {
        self.release_first_run.notify_waiters();
    }
}

#[derive(Clone)]
pub(super) struct AdmissionGatedHost {
    pub(super) capabilities: WorkflowHostCapabilities,
    pub(super) admission_open: Arc<AtomicBool>,
}

impl AdmissionGatedHost {
    pub(super) fn new(admission_open: Arc<AtomicBool>) -> Self {
        Self {
            capabilities: MockWorkflowHost::new(8, 1024).capabilities,
            admission_open,
        }
...
pub(super) struct RecordingRuntimeHost {
    pub(super) retention_hints: Arc<Mutex<Vec<WorkflowSessionRetentionHint>>>,
    pub(super) capabilities: WorkflowHostCapabilities,
}

impl RecordingRuntimeHost {
    pub(super) fn new(retention_hints: Arc<Mutex<Vec<WorkflowSessionRetentionHint>>>) -> Self {
        Self {
            retention_hints,
            capabilities: WorkflowHostCapabilities {
                max_input_bindings: 16,
...
pub(super) struct SelectingRuntimeHost {
    pub(super) selected_session_id: String,
    pub(super) unloads: Arc<Mutex<Vec<(String, WorkflowSessionUnloadReason)>>>,
    pub(super) capabilities: WorkflowHostCapabilities,
}
...
pub(super) struct AffinityRuntimeHost {
    pub(super) unloads: Arc<Mutex<Vec<String>>>,
    pub(super) capabilities: WorkflowHostCapabilities,
    pub(super) required_backends_by_workflow: HashMap<String, Vec<String>>,
    pub(super) required_models_by_workflow: HashMap<String, Vec<String>>,
}

impl AffinityRuntimeHost {
    pub(super) fn new(unloads: Arc<Mutex<Vec<String>>>) -> Self {
        Self {
            unloads,
            capabilities: WorkflowHostCapabilities {
                max_input_bindings: 16,
    ...
    pub(super) fn with_runtime_affinity(
...
pub(super) struct PreflightHost {
    pub(super) capabilities: WorkflowHostCapabilities,
    pub(super) technical_fit_decision: Option<WorkflowTechnicalFitDecision>,
}
...
pub(super) struct DefaultCapabilitiesHost {
    pub(super) workflow_root: PathBuf,
}

pub(super) struct CountingPreflightHost {
    pub(super) workflow_capabilities_calls: Arc<AtomicUsize>,
    pub(super) runtime_capabilities_calls: Arc<AtomicUsize>,
    pub(super) graph_fingerprint: Arc<Mutex<String>>,
    pub(super) technical_fit_requests: Arc<Mutex<Vec<WorkflowTechnicalFitRequest>>>,
}

#[derive(Clone)]
pub(super) struct MockSchedulerDiagnosticsProvider {
    pub(super) diagnostics: WorkflowSchedulerRuntimeRegistryDiagnostics,
    pub(super) requests: Arc<Mutex<Vec<WorkflowSchedulerRuntimeDiagnosticsRequest>>>,
}
