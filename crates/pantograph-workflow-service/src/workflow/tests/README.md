...
| `session_admission.rs` | Queue admission tests for canonical cold-start behavior without legacy runtime-load gates. |
...
- Diagnostics projection tests must verify run-list client/session/bucket,
  accepted-at range, and workflow execution-session scope facts through public
  query responses instead of raw event inspection.
...
- Session execution tests with diagnostics enabled must verify scheduler
  ...
  reasons, typed blocking conditions, and typed candidate runtime ids instead
  of deriving estimate text from mutable graph state.
- Session execution tests with diagnostics enabled must verify admission events
  include selected runtime and reserved model facts from scheduler/dequeued-run
  state instead of current graph reads.
- Session execution tests with diagnostics enabled must verify local
  runtime-slot reservation created/released events bracket started/terminal
  execution visibility without replacing `run.*` lifecycle facts.
