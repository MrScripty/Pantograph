...
| `task_execution_runtime.rs` | Internal composition-root runtime owner that holds the workflow service plus task-execution worker lifecycle without making `WorkflowService` self-referential; it exposes typed worker unavailable/shutdown outcomes for future request-facade enqueue paths. |
