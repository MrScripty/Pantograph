...
| `task_execution_facade.rs` | Production composition-root session execution entrypoint that owns the shared workflow service plus task-execution runtime owner so worker lifecycle can be routed without making `WorkflowService` self-referential. |
