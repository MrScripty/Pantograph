...
| `task_execution_facade.rs` | Production composition-root session execution entrypoint that owns the shared workflow service plus task-execution runtime owner and delegates existing session execution before worker-owned runtime branch migration, without making `WorkflowService` self-referential. |
| `task_execution_runtime.rs` | Internal composition-root runtime owner that holds the workflow service plus task-execution worker lifecycle without making `WorkflowService` self-referential; it exposes typed worker unavailable/shutdown outcomes and runtime-branch execution context for future request-facade enqueue paths. |
...
| `task_execution_worker.rs` | Internal task-execution worker command, outcome, and diagnostic contracts for backend-owned task-attempt and runtime-branch execution loops. |
