...
| `graph_api.rs` | Graph edit-session, mutation, connection, persistence, backend validation task trigger/shutdown, and runtime snapshot facade methods. |
...
| `task_execution_facade.rs` | Production composition-root session execution entrypoint that owns the shared workflow service, host/runtime boundary, and task-execution runtime owner before worker-owned runtime branch migration, without making `WorkflowService` self-referential. |
| `task_execution_runtime.rs` | Internal composition-root runtime owner that holds the workflow service plus task-execution worker lifecycle without making `WorkflowService` self-referential; it passes the shared backend runtime-branch environment into worker spawn, enqueues runtime branches with real completion responders, and exposes typed worker unavailable/shutdown outcomes. |
...
| `task_execution_worker.rs` | Internal task-execution worker command, runtime-branch environment, completion responder, outcome, and diagnostic contracts for backend-owned task-attempt and runtime-branch execution loops. |
...
freshness.
Runtime scheduler task graphs carry a typed `dependency_readiness_source`
...
- Workflow service owns the shared dependency requirements registry used by
  ...
  valid resolved or ready `ValidatedDependencyEnvironmentResult` values, or
  from an equivalent already-validated `DependencyRequirementsPayload` at the
  canonical backend boundary. Workflow-service seeds the registry before it
  enqueues dependency-readiness work; producers then perform check/install host
  probes from the registry payload. Workflow-service must not reconstruct
  registry payloads from requirements-id strings, graph/editor/frontend state,
