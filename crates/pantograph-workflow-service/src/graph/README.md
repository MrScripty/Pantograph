...
| `session_inference_validation_api.rs` | Edit-session inference-validation publication API that records only current graph-revision validation summaries and exposes backend-owned lifecycle event snapshots for transport read models. |
...
- Validation publication attempts use the dedicated workflow-service publisher
  ...
  cancellation while fact lookup is pending and again after fact lookup
  completes, then stops before publication acceptance or current-state recording
  when a session was superseded or closed.
...
- Validation-session lifecycle identity is workflow-service owned. Starting a
  ...
  starting a replacement validation session, mutating the graph revision, or
  closing the graph edit session signals typed cancellation before stale
  provider results can publish.
