...
| `runtime_dispatch_candidate_provider.rs` | Owns the embedded-runtime workflow-service dispatch candidate provider boundary and synchronous source-snapshot contract for already-collected Pumas package and runtime capability outcomes. The initial provider fails closed with typed diagnostics when staged Pumas package, runtime capability, or resource facts are unavailable, and it must not emit resource-backed candidates until fact assembly and reservation lifecycle coverage are wired. |
