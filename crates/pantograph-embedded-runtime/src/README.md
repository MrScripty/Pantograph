...
| `runtime_host_media_artifact_sink.rs` | Owns the runtime-host media output persistence boundary. It writes generated image bytes through the backend workflow-service ArtifactStore and returns path-free runtime-host media refs, while malformed images or unavailable artifact persistence fail closed with typed sink errors. |
