...
| `embedded_runtime_lifecycle.rs` | Owns embedded-runtime constructors, hosted-composition runtime wrapping, host wiring, dependency-readiness snapshot provider and auto-resume lifecycle composition for standalone services, registry injection, accessors, and shutdown coordination, including workflow-service validation task shutdown. |
...
| `inference_interface_facts_provider.rs` | Owns the backend-composed inference-interface facts provider that combines Pumas package facts, runtime-registry capability facts, and conservative resource estimates into workflow-service resolver facts without graph/path/UI/Tauri fallback. |
| `inference_resource_estimator.rs` | Owns the pure Pantograph conservative resource-estimate transform from validated Pumas logical-size facts into inference estimate DTOs and scheduler estimate hints, without graph/path/UI fallback, runtime registry I/O, or Tauri policy. |
...
| `dependency_readiness_auto_resume.rs` | Owns the tracked dependency-readiness auto-resume lifecycle, including poll-interval validation, backend resume-candidate polling, duplicate candidate suppression, non-terminal pending-readiness handling, owner-level logging, and idempotent shutdown. |
| `dependency_readiness_auto_resume_tests.rs` | Focused lifecycle tests for auto-resume shutdown, empty candidate polling, successful resume attempts, duplicate candidate suppression, pending-readiness preservation, and poll-interval validation. |
...
| `runtime_dispatch_candidate_provider.rs` | Owns the embedded-runtime workflow-service dispatch candidate provider boundary, synchronous source-snapshot contract for already-collected Pumas package/runtime capability outcomes, and internal path-free candidate-draft assembly from matched backend hints. The initial provider fails closed with typed diagnostics when staged Pumas package, runtime capability, or resource facts are unavailable, and it must not emit resource-backed candidates until reservation lifecycle coverage is wired. |
...
| `runtime_dispatch_source_snapshot.rs` | Owns the versioned embedded-runtime dispatch source-fact snapshot bridge used before the later readiness/admission-attached persisted snapshot. It refreshes Pumas package facts and runtime capability facts through their source owners, implements the workflow-service pre-dispatch source-refresh port, validates freshness/model-ref/path-free constraints, and returns typed diagnostics instead of exposing stale or mismatched source facts to the synchronous dispatch provider. |
| `runtime_extensions.rs` | Owns shared runtime extension snapshots and executor extension injection for Pumas owner API, explicit Pumas selector access, KV cache, event sinks, execution ids, Python runtime execution records, and host-provided inference lifecycle sinks. It must not propagate the retired model dependency resolver into runtime execution. |
...
| `runtime_host_execution_port.rs` | Owns the embedded-runtime implementation of the shared runtime-host execution ports. The single-task port validates dispatch-selected scheduler handoff requests, consumes narrow package-facts, load-target, gateway, and media-artifact sink dependencies, completes image-generation execution when those dependencies are injected, and fails closed with typed diagnostics when required dependencies or execution steps are unavailable. The batch port validates grouped scheduler handoff requests, resolves shared package/load-target facts through the anchor member after compatibility checks, plans each member from its own materialized inputs, calls the inference gateway batch operation once, writes member artifacts independently, and never delegates to the single-task port as fallback. |
| `runtime_host_image_execution.rs` | Owns the backend-only projection from a validated scheduler runtime-host image request, scheduler dispatch decision, full Pumas package facts, and Pumas load target into canonical inference image planning input. The boundary fails closed for unsupported tasks, runtimes, devices, and materialized ports, and remains unwired from successful execution until package-facts and backend-decision ownership are resolved. |
| `runtime_host_package_facts.rs` | Owns host-only full Pumas package-facts resolution for runtime-host execution from the scheduler-selected model ref, including Pumas-to-inference contract decoding and typed fail-closed diagnostics for missing dispatch decisions, stale facts, decode failures, and selected-artifact mismatches. |
...
| `runtime_host_media_artifact_sink.rs` | Owns the runtime-host media output persistence boundary. It writes generated image bytes through the shared backend workflow artifact writer and returns path-free runtime-host media refs, while malformed images or unavailable artifact persistence fail closed with typed sink errors. |
...
| `workflow_service_composition.rs` | Owns embedded-runtime workflow-service and hosted startup construction before the service is shared, attaching dependency-readiness components, hosted resource-backed runtime dispatch dependencies, shared artifact-writer-backed runtime-host media output, scheduler diagnostics, executor extensions, dependency-readiness lifecycle handles, and capacity limits at the composition boundary so runtime dispatch wiring does not require mutable post-share service mutation. Legacy dependency resolver activity remains a diagnostic/tooling cleanup target and must not be installed into runtime execution extensions. |
