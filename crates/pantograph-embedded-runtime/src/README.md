...
| `runtime_host_execution_port.rs` | Owns the embedded-runtime implementation of the shared runtime-host execution port. The initial port validates dispatch-selected scheduler handoff requests and fails closed with typed diagnostics until host-owned Pumas load-target resolution and runtime-specific execution are wired together. |
