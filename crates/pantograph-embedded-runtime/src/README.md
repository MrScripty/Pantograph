...
| `dependency_inventory_tests.rs` | Focused tests for per-selected-binding inventory dispatch through the Python, managed-runtime, and runtime-feature providers, explicit not-implemented rows for selected unsupported bindings, mixed provider payload projection, and shared observation projection into readiness snapshots. |
