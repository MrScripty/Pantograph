...
fn node_execution_workflow_sink_records_resolved_inputs_as_retained_node_artifacts() {
    ...
    assert_eq!(artifacts.artifacts.len(), 1);
    let artifact = &artifacts.artifacts[0];
    assert_eq!(artifact.consumer_node_id.as_deref(), Some("node-a"));
    assert_eq!(artifact.consumer_port_id.as_deref(), Some("prompt"));
    assert!(artifact.producer_node_id.is_none());
    assert_eq!(artifact.media_type.as_deref(), Some("text/plain"));
    ...
    let body = service
        ...
        .expect("artifact body should be readable");
    assert_eq!(body.body, b"retained prompt text");
}
