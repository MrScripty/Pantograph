...
#[test]
fn run_terminal_projects_canonical_error_link_without_counting_new_error() {
    let mut ledger = SqliteDiagnosticsLedger::open_in_memory().expect("ledger opens");
    ledger
        .append_diagnostic_event(sample_run_snapshot_event("workflow_run_alpha"))
        .expect("run snapshot event");
    ledger
        .append_diagnostic_event(sample_run_started_event("workflow_run_alpha"))
        .expect("run started event");
    let mut terminal = sample_run_terminal_event("workflow_run_alpha");
    ...
    assert_eq!(runs.len(), 1);
    let run = &runs[0];
    assert_eq!(run.status, RunListProjectionStatus::Failed);
    assert_eq!(
        run.latest_error_event_id.as_deref(),
        Some("diagnostic-error-runtime-alpha")
    );
    assert_eq!(
        run.latest_error_message.as_deref(),
        Some("runtime failed after diagnostic error")
    );
...
fn scheduler_timeline_projection_drains_events_incrementally() {
    ...
    assert_eq!(
        ...
        Some(
            "queue wait 10 ms; warm_session_reused; selected runtime llama_cpp; selected runtime variant llama_cpp.cuda; selected backend llama_cpp; reserved model(s): model-alpha"
        )
    );
...
fn sample_scheduler_admission_event(workflow_run_id: &str) -> DiagnosticEventAppendRequest {
    DiagnosticEventAppendRequest {
        source_component: DiagnosticEventSourceComponent::Scheduler,
        source_instance_id: Some("scheduler-local".to_string()),
        ...
        payload: DiagnosticEventPayload::InferenceExecutionDiagnosticObserved(
            InferenceExecutionDiagnosticObservedPayload {
                ...
                kv_cache: Some(InferenceKvCacheDiagnosticSummary {
                    ...
                    cache_id: Some("kv-cache-1".to_string()),
