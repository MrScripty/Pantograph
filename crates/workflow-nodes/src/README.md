...
| `contracts.rs` | Canonical primitive contract projection for built-in workflow nodes. |
...
- Experimental control/tool nodes must not be presented as complete execution
  behavior while tool execution is disabled. `tool-loop` remains a stable
  authoring surface, but its executable design is now scheduler-owned agent
  loop expansion, not a composed wrapper over static `llm-inference` ports.
...
- Tool-loop/tool-executor behavior gains scheduler-owned agent-loop/tool
  runtime execution or is removed from the descriptor set.
