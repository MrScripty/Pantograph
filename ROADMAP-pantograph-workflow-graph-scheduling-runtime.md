...
- Producer stop composition now also invalidates backend workflow-session
  ...
  not retain stale `runtime_loaded` state after producers are torn down and
  the next run is forced back through the shared preflight/load contract.
...
- `node-engine` now also exposes explicit workflow-session binding on
  ...
  run/execution ids.
- The single-demand and multi-demand executor paths now both record bound-
  session node-memory snapshots from backend cache state after successful
