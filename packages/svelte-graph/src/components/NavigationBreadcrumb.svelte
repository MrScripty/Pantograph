<script lang="ts">
  ...
  import type { BreadcrumbItem } from '../types/view.js';

  const { stores } = useGraphContext();
  const { breadcrumb, isAnimating, canNavigateBack, viewLevel } = stores.view;
  const { navigateToBreadcrumb, navigateBack } = stores.view;
...
>
  ...
  <ol class="breadcrumb-list">
    {#each $breadcrumb as item, index (item.id)}
      <li class="breadcrumb-item" in:fly={{ x: -10, duration: 200, delay: index * 50 }}>
        {#if index > 0}
        ...
        >
          <span class="level-icon" aria-hidden="true">{levelIcons[item.level] || '\uD83D\uDCC4'}</span>
