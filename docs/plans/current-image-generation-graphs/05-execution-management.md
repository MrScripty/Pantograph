...
- 2026-05-25 Milestone 5d inference `node.data.definition` fallback rejection
  ...
  - Remaining follow-up: project authored inference interface snapshots into
    ...
    fallback source.
- 2026-05-25 Milestone 5d authored inference snapshot effective-port
  projection slice completed:
  - Smallest useful vertical slice: added workflow-service projection from
    ...
    unknown future snapshot value/requirement variants until the graph contract
  ...
  - No-fallback/no-legacy confirmation: authored snapshots now have a canonical
    ...
    `node.data.definition`, Pumas package facts, runtime load targets, model
    paths, or scheduler decisions as inference interface sources.
  - Verification passed: `cargo fmt -p pantograph-workflow-service --
    --check`; `cargo test -p pantograph-workflow-service
    graph::effective_definition --lib`; `cargo test -p
    pantograph-workflow-service
    ...
    `cargo check -p pantograph-workflow-service`; `cargo check -p
    pantograph-workflow-service --no-default-features`; `cargo check -p
    pantograph-workflow-service --all-features`; and `git diff --check`. The
    workflow-service check commands still report only the known
...
- 2026-05-25 Milestone 5d live validation event node-identity re-plan
  boundary:
  ...
  - Why implementation stops: wiring extracted graph requests into live
    validation events without node identity would require a single-node
    assumption or implicit event ordering. Both violate the canonical
    multi-inference design and would create a hidden compatibility rule that
    scheduler batching/distribution work would later need to undo.
  - Re-plan options to decide:
    1. Add `node_id` to every `WorkflowGraphInferenceValidationEvent` envelope.
       ...
       identity or need a sentinel, weakening the event model.
    2. Add `node_id` only to node-scoped payload variants such as descriptor,
       drift, diagnostic, and update proposal, while keeping graph summary
       events graph-scoped. This is explicit but spreads routing identity
...
- 2026-05-26 Milestone 5d workflow-service executable publish snapshot slice
  completed:
  - Added a typed executable publish request and service facade method that
    checks the validation publication graph revision against the graph being
    published, resolves the immutable workflow version, compacts the publication
    into the executable validation snapshot DTO, and persists it through
    attribution-owned opaque snapshot storage.
  - No-fallback/no-legacy result: draft graph save remains separate from
    executable publish, stale validation publications fail closed, and publish
    does not reconstruct executable authority from graph fields, frontend/Tauri
...
- 2026-05-26 Milestone 5d dependency requirements proof producer re-plan
  ...
  - Discovery: `pantograph-dependency-planning` currently provides typed
    ...
    ids.
  - Decision: choose option 1. Add the producer API in
    `pantograph-dependency-planning` now, then have workflow-service call it for
    `Resolve` and store the returned proof through the current validation-state
    proof API.
  - Ownership: `pantograph-dependency-planning` owns requirements identity,
    proof status, and dependency-planning diagnostics. Workflow-service owns
    graph/session freshness and stores the current proof; it must not synthesize
    hidden ids or derive dependency-planning policy.
  - Slice plan:
    1. Add the dependency-planning producer contract and tests in
       `pantograph-dependency-planning`. The producer is pure contract/domain
       logic unless supplied typed availability facts: it derives a stable
       path-free requirements id/proof from validated request identity,
       platform key, selected binding ids, dependency override fingerprint,
       runtime/device constraints, dependency-planning-local trait intents, and
       optional typed availability facts. It must not call Pumas, inspect
       files, select runtime/device policy, infer package readiness from
       ambient local state, or depend on scheduler DTOs.
    2. Use canonical typed input plus a stable hash for
       `DependencyRequirementsId`; do not concatenate raw model/user strings or
       rely on local filesystem/path material.
    3. Wire workflow-service `Resolve` to snapshot the graph, resolve the
       sidecar subject, extract and validate sidecar-authored selected binding
       ids and manual override patches, release graph state, call the producer,
       and store the returned proof. `Check` and `Install` remain fail-closed
       unless a current proof exists.
    4. Store dependency-planning diagnostics in the proof and map them to
       `InferenceInterfaceDiagnostic` only at graph/action response boundaries.
    5. Derive canonical `DependencyEnvironmentRequest` only after the producer
       proof and current executable descriptor summary agree.
  - Standards gates added on 2026-05-26: producer DTOs/status/diagnostics/errors
    must be public dependency-planning contract types re-exported from `lib.rs`,
    validated through typed constructors or `TryFrom`, and extension-ready
    where future runtimes, model families, or availability states are expected.
    Pure producer logic stays synchronous and does not use `Result<T, String>`,
    `anyhow`, raw metadata maps, or async APIs without an I/O justification.
    Workflow-service must parse sidecar JSON ports once into validated
    dependency-planning types, release graph locks before producer/fact work,
    and recheck graph revision plus validation session before recording a
    proof. Stable hashing must follow dependency standards: if `blake3` is used
    by the producer, first centralize the existing workspace `blake3`
    declarations under `[workspace.dependencies]` and switch member crates to
    `{ workspace = true }` in the same atomic dependency-hygiene slice.
    Implementation must update relevant READMEs for new producer/module
    contracts and include contract, workflow-service, diagnostic mapping, stale
    proof, and cross-layer acceptance tests.
  - No-fallback/no-legacy confirmation: the proof remains path-free and may not
    carry executable paths, package facts, runtime load targets, frontend
    display state, media payloads, scheduler trait-setting DTOs, or scheduler
    dispatch decisions. Retired dependency action payload builders and path-era
    dependency ownership remain deletion targets, not compatibility layers.
  - Re-plan triggers: stop if producing the proof requires Pumas path
    resolution, runtime-host load targets, scheduler queue state, frontend/Tauri
    authority changes, workflow-service-local requirements id synthesis,
    dependency availability discovery without typed availability facts, or
    scheduler DTOs entering `pantograph-dependency-planning`.
- 2026-05-26 Milestone 5d dependency-planning producer contract slice:
  - Slice: add the pure backend dependency requirements proof producer in
    `pantograph-dependency-planning` and keep workflow-service proof production
    deferred to the next slice.
  - Allowed write set used: workspace and member Cargo manifests/lockfile for
    `blake3` dependency hygiene; `pantograph-dependency-planning` source, tests,
    and READMEs; node-engine request construction for the new typed trait
    intent field; plan ledger files.
  - No-fallback/no-legacy result: the producer derives requirements identity
    from canonical typed fields and rejects path-carrying Pumas refs. It does
    not call Pumas, inspect files, infer package readiness from local state,
    select runtime/device policy, accept scheduler DTOs, or use frontend/Tauri
    display state.
  - Behavior: `DependencyRequirementsProof` now carries a path-free identity
    key, stable requirements id, stable override fingerprint, typed trait
    intents, proof status, and dependency-planning diagnostics. Optional typed
    availability facts map `DependencyPlanningState` to proof status; absent
    facts produce a current proof for a validated path-free request.
  - Dependency hygiene: `blake3` was centralized in root
    `[workspace.dependencies]`; existing member declarations now use
    `{ workspace = true }`; `pantograph-dependency-planning` declares direct
    ownership because it now hashes requirements identity.
  - Verification passed: `cargo fmt`; `cargo test -p
    pantograph-dependency-planning`; `cargo check -p
    pantograph-dependency-planning -p node-engine`.
  - Deviation/discovery: the existing dependency planning request fixture still
    carries a migration-era `selected_artifact_path`; producer tests clear it
    for path-free production and include an explicit rejection test for
    path-carrying identity.
  - Remaining follow-up: wire workflow-service `Resolve` to call the producer,
    parse sidecar selected bindings and manual overrides once into typed
    dependency-planning values, preserve dependency-planning diagnostics in the
...
- 2026-05-26 Milestone 5d dependency-environment service owner decision:
  - Decision: use option 2. Add a dedicated backend dependency-environment
    service crate, suggested name `pantograph-dependency-environment-service`,
    as the canonical owner of
    `ValidatedDependencyEnvironmentRequest -> DependencyEnvironmentResult`.
  - Ownership: workflow-service derives and validates the canonical request
    from current graph/descriptor/dependency-planning state, calls the service,
    and projects typed results into action responses or backend-owned display
    state. Tauri remains transport-only, graph editor callers continue to send
    action intents only, and node-engine/embedded-runtime do not execute
    dependency-environment actions.
  - Service constraints: consume canonical dependency-planning DTOs and
    Pumas-approved dependency/environment APIs only. Do not accept
    `ModelDependencyRequest`, `model_path`, local paths, package-fact blobs,
    frontend platform context, or graph display metadata. Missing Pumas support
    must return typed unavailable/not-implemented
    `DependencyEnvironmentResult` diagnostics rather than falling back to the
    retired resolver path.
  - Planned thin slices:
    1. Add the dedicated crate and public service API around validated request
       and result DTOs, including the validated result wrapper or service-owned
       constructors.
    2. Add a no-I/O/null implementation or fake provider for contract tests
       proving validation, result invariants, diagnostics, no path-shaped
       fields, and no dependency on legacy resolver crates.
    3. Wire workflow-service dependency action handling to call the service
       from a lock-free snapshot after canonical request derivation, rejecting
       stale graph revisions/session ids before publishing result state.
    4. Delete or replace active dependency-environment execution through
       node-engine/embedded-runtime `ModelDependencyRequest`; do not add a
       compatibility adapter.
    5. Remove Tauri model-dependency command usage from the
       dependency-environment action path and verify the frontend sends only
       descriptor action intents.
    6. Feed scheduler dependency readiness/admission from canonical dependency
       results rather than graph-authored dependency-environment outputs.
- 2026-05-26 Milestone 5d dependency-environment service owner blast-radius
  tightening:
  - Findings accepted: the service-owner plan is directionally correct, but
    implementation needs explicit standards gates for lock handling, result
    validation, crate dependencies, legacy deletion, scheduler admission shape,
    and lifecycle ownership.
  - Service API rule: `pantograph-dependency-environment-service` must expose a
    narrow boundary over `pantograph-dependency-planning` DTOs and provider
    traits only. It must not depend on workflow-service, node-engine,
    embedded-runtime, Tauri, frontend DTOs, or graph display state. Keep pure
    validation/projection synchronous; async is allowed only at concrete
    provider methods that perform Pumas calls, installs, file inspection,
    process spawning, or other I/O.
  - Result contract rule: add a validated dependency-environment result
    boundary before production callers trust service output. This may be a
    `ValidatedDependencyEnvironmentResult` wrapper or service-owned
    constructors, but it must enforce ready identity/proof fields,
    operation/status compatibility, unique selected binding identifiers,
    explicit unavailable/not-implemented diagnostics, and absence of
    path-shaped values.
  - Workflow-service rule: dependency-environment action handling must snapshot
    graph/session state under lock, derive the validated request from that
    snapshot, release locks before service/provider work, then reacquire only
    to publish the result when the graph revision/session id still matches. Do
    not await Pumas, install, file, or service work while holding validation or
    session locks.
  - Legacy deletion rule: retire frontend request-building helpers that gather
    `modelPath`, backend keys, platform context, package facts, or
    graph-authored requirements for dependency-environment actions. Remove
    dependency-environment execution through embedded-runtime/node-engine
    `ModelDependencyRequest` rather than wrapping it, and verify no
    dependency-environment action path calls Tauri model-dependency commands.
  - Scheduler/lifecycle rule: scheduler readiness must consume a canonical
    dependency readiness/admission proof derived from validated dependency
    results, not graph node outputs or frontend display state. Dependency
    resolve/check/install lifecycle state, status caches, operation
    de-duplication, and install locks belong to the service or its concrete
    provider; Tauri remains transport-only.
...
- 2026-05-26 Milestone 5d dependency-environment service owner standards
  iteration:
  - Standards reviewed:
    `/media/jeremy/OrangeCream/Linux Software/repos/owned/developer-tooling/Coding-Standards/PLAN-STANDARDS.md`,
    `ARCHITECTURE-PATTERNS.md`, `DOCUMENTATION-STANDARDS.md`,
    `TESTING-STANDARDS.md`, `DEPENDENCY-STANDARDS.md`,
    `CONCURRENCY-STANDARDS.md`, `languages/rust/RUST-API-STANDARDS.md`,
    `languages/rust/RUST-ASYNC-STANDARDS.md`, and
    `languages/rust/RUST-DEPENDENCY-STANDARDS.md`.
  - Rust API gate added: public service APIs must use typed request/result,
    operation, status, diagnostic, and identity contracts instead of raw
    strings or path-shaped primitives. Public fallible library APIs must return
    structured error enums, not `Result<T, String>` or `anyhow`, and must avoid
    `unwrap`/`expect` in request, lifecycle, and provider paths. Extensible
    public enums/structs should be `#[non_exhaustive]`; validated values and
    builders should be `#[must_use]`; validation-required constructors stay
    private.
  - Dependency gate added: new dependencies require Cargo tree inspection,
    narrow owner declaration in the crate that directly uses them, `std` or
    existing workspace dependency reuse for small behavior, and feature gates or
    written justification for heavy/platform-specific provider dependencies.
    The service crate must not pull in broad runtime/framework dependencies to
    expose the core contract.
  - Documentation gate added: the new crate needs crate-level Rust docs and
    README/module README coverage for every new `src/` directory, including
    purpose, invariants, API consumer contract, lifecycle/error semantics,
    rejected alternatives, and revisit triggers. Existing source directories
    touched by the slice must update their README or record ADR/plan
    traceability.
  - Verification gate added: slices need public-API tests for validated
    request/result construction and typed diagnostics, plus a thin vertical
    slice proving workflow-service derives the request, calls the fake/null
    service without holding graph/session locks, rejects stale graph
    revisions/session ids, and publishes only backend-owned display/status
    state. Tests touching caches, install locks, temp files, or process-global
    state must isolate durable resources or serialize with a documented guard.
    Source-search checks must prove retired path-shaped fields and commands are
    absent from the dependency-environment action path.
  - Background-work gate added: the service crate must not create a global
    async runtime or spawn detached tasks. Future provider installs, polling,
...
- 2026-05-26 Milestone 5d active run-loop readiness caller re-plan boundary:
  ...
  - No-fallback/no-legacy confirmation: do not wire active execution by
    ...
    of local paths.
- 2026-05-27 Milestone 5d active run-loop readiness freshness design:
  - Decision: use option 2. Add a separate path-free execution freshness
    envelope in `pantograph-dependency-planning` while keeping
    `DependencyPlanningIdentityKey` scoped to stable dependency identity.
    Workflow-service builds the envelope from the admitted scheduler task and
    current executable validation state; scheduler admission validates the
    envelope before any runtime task can move from `WaitingDependencyReadiness`
    to `Ready`.
  - Ownership tightening from code review: `pantograph-dependency-planning`
    must own validated envelope newtypes instead of importing
    inference-interface or scheduler-owned graph/session/task types.
    Workflow-service owns adapters from executable validation snapshots,
    active-run task graph state, and scheduler task records into those shared
    envelope contracts.
  - Required DTOs: add validated request/proof envelope contracts carrying
    workflow/run/task/node identity, graph revision, validation session or
    executable snapshot identity, descriptor fingerprint, dependency
    requirements id, selected binding ids, dependency override fingerprint,
    readiness proof id/version, and correlation id. They must not carry paths,
    Pumas package facts, load targets, artifact paths, worker settings,
    frontend display state, or runtime-host payloads.
  - Freshness/proof retention requirements: queue admission must retain a
    workflow-service owned freshness context containing executable validation
    snapshot identity, graph revision, validation session id, descriptor
    fingerprint, selected binding ids, dependency requirements id, and
    dependency override fingerprint. Scheduler admission/dispatch proof must
    carry fingerprints and bounded diagnostics only, not raw override patches
    or provider-private request payloads. The admitted proof envelope must be
    retained with the ready runtime task, or in a workflow-service owned
    active-run proof store keyed by task/state identity, before dispatch
    selection can expose the task.
  - Standards compliance tightening: dependency-planning validation remains the
    sync core and exposes validated newtypes plus typed errors; workflow-service
    owns async provider/store orchestration without holding locks across awaits;
    proof retention must be atomic with the scheduler state transition; all
    public envelope contracts carry explicit versions, serde fixtures,
    unknown-field rejection, bounded typed diagnostics, and README/API Consumer
    Contract updates; no new dependency may be added without dependency-standard
    cost/ownership justification.
  - Validation and staging: dependency-planning gets serde/negative field and
    replay diagnostics first; workflow-service then exposes the freshness source
    fields through an owner API and wraps readiness requests/proofs; scheduler
    replaces bare `SchedulerDependencyReadinessProof` with envelope-aware proof
    contracts; scheduler tests reject stale, mismatched, replayed, cross-run,
    and cross-task proofs and verify proof retention across the transition to
    `Ready`; replay/idempotency, active-run recovery, concurrent-workflow
    isolation, and persisted-state fixture tests are added where the slice
    touches those surfaces; scheduler README text is updated with the new
    retention invariant; the active run loop is wired only after the envelope
    path is validated. The old runtime-dispatch-not-wired terminal path is a
    replacement target, not a compatibility fallback.
- 2026-05-27 Milestone 5d dependency readiness execution envelope contract
  slice:
  - Smallest useful vertical slice: added only the
    `pantograph-dependency-planning` path-free execution freshness contracts,
    validated wrappers, fixtures, and public integration tests. Touched files
    were limited to dependency-planning source/tests/fixtures/READMEs plus the
    Milestone 5d plan and this execution log.
  - No-fallback/no-legacy confirmation: the slice adds canonical scheduler
    readiness proof envelope contracts without wiring legacy runtime dispatch,
    graph/front-end proof reconstruction, executable path identity, Pumas load
    targets, or runtime-host payloads into scheduler proof.
  - Implementation completed: added dependency-planning-owned newtypes for
    active-run freshness ids, `DependencyReadinessExecutionContext`,
    `DependencyReadinessRequestEnvelope`,
    `DependencyReadinessProofEnvelope`, and validated wrappers; added fixtures
    and tests for valid request/proof envelopes plus unknown-field, missing
    validation identity, caller-context drift, requirements drift, zero proof
    version, executable handoff field, and raw request payload rejection.
  - Verification passed: `cargo fmt -p pantograph-dependency-planning`,
    ...
    pantograph-dependency-planning`, `git diff --check`, and targeted source
  ...
  - Remaining follow-up: workflow-service must next expose the queue-admission
...
- 2026-05-27 Milestone 5d saved-run executable snapshot publish submit slice:
  ...
  - Verification passed: `npm run typecheck`; `node
    ...
    --check`.
  - 2026-05-27 re-plan decision: the next slice is option 3, a backend-owned
    current validation summary read model/API for `graph_session_id +
    graph_revision` that feeds the graph editor Submit disabled reason. The
    summary must expose current/pending/missing/stale/unavailable/invalid and
    executable/not-executable status, current `validation_session_id`,
    diagnostics, and graph revision without letting Tauri/frontend resolve
    descriptors, Pumas facts, runtime facts, dependency proofs, or scheduler
    policy. Publish-before-run remains the fail-closed admission guard.
  - 2026-05-27 investigation tightening recorded: the option 3 summary API must
    be a cheap read of workflow-service current validation state rather than a
    descriptor/Pumas/runtime/dependency re-resolution path. Missing, pending,
    stale, unavailable, invalid, and non-executable editor states are typed
    response states, not transport failures. The response owns the Submit gate
    with `allowed`, typed reason code, and display message; Tauri stays a thin
    wrapper; TypeScript mirrors are strongly typed; and saved-run Submit must
    pass the accepted summary's `validation_session_id` into the executable
    snapshot publisher so graph/session races fail closed.
  - 2026-05-27 standards iteration recorded: the option 3 implementation slice
    must include focused workflow-service tests for all typed summary/gate
    states and validation-session mismatch; Tauri command tests; TypeScript
    service DTO tests; toolbar/store tests proving Submit follows backend
    `submit_gate`; one thin cross-layer summary-to-submit-publisher acceptance
    check; affected README updates; and source-search checks proving Tauri/
    frontend do not resolve descriptors, Pumas/runtime facts, dependency
    proofs, or scheduler policy. Option 3 must not introduce frontend polling
    or background validation workers; any temporary refresh helper must be
    graph-session/revision scoped and cleaned up on graph/session change.
  - Future target recorded for option 4: event-driven live validation should
    reuse the option 3 summary DTO and state owner, then replace explicit
    refresh/query mechanics with workflow-service-owned validation events.
    Graph edits must remain nonblocking; stale sessions must be discarded by
    graph session/revision/validation identity; node-scoped events carry
    descriptor, drift, diagnostic, and update-proposal payloads; graph-scoped
    events carry summary status; and the editor disables Submit from the latest
    backend summary rather than local inference.
  - Option 4 preservation note recorded: event-driven validation may replace
    explicit query mechanics, but must publish the same backend-owned
    summary/gate shape with graph session, graph revision, validation session,
    and monotonic event-sequence identity. It must not create a second
    submit-gate authority, frontend polling policy loop, or frontend-owned
    validation interpretation.
  - Option 4 async lifecycle standards recorded: workflow-service must own any
    spawned validation tasks, tracked handles, cancellation tokens, bounded
    event buffers, supersession rules, shutdown/session-close cleanup, and
...
- 2026-05-28 Milestone 5d validation node projection bound slice:
  ...
  - Discovered issue: `cargo check -p pantograph-workflow-service` still
    reports the pre-existing `set_active_run_execution_plan` dead-code warning.
- 2026-05-28 Milestone 5d validation lifecycle cancellation signal slice:
  - Smallest useful vertical slice: add owner-issued cancellation receivers to
    active validation lifecycle records so supersession and graph-session close
    can signal cancellation before stale provider results publish.
  - Allowed files touched:
    `crates/pantograph-workflow-service/src/graph/inference_validation_lifecycle.rs`,
    `crates/pantograph-workflow-service/src/graph/README.md`,
    the Milestone 5d file, and this execution log.
  - No-fallback/no-legacy result: cancellation remains workflow-service owned
    and typed by graph session, graph revision, and validation-session ids. No
    detached tasks, transport cancellation policy, frontend state, or alternate
  ...
  - Discovered issue: `cargo check -p pantograph-workflow-service` still
    reports the pre-existing `set_active_run_execution_plan` dead-code warning.
- 2026-05-28 Milestone 5d validation lifecycle typed cancellation reason slice:
  - Smallest useful vertical slice: replace boolean lifecycle cancellation with
    typed cancellation reasons for supersession and graph-session close.
  - Allowed files touched:
    `crates/pantograph-workflow-service/src/graph/inference_validation_lifecycle.rs`,
    `crates/pantograph-workflow-service/src/graph/README.md`,
    the Milestone 5d file, and this execution log.
  - No-fallback/no-legacy result: cancellation causes stay workflow-service
    owned and typed instead of being inferred from strings, transport state,
    frontend policy, or incidental metadata.
  - Verification passed: `cargo fmt -p pantograph-workflow-service --
    --check`; `cargo test -p pantograph-workflow-service
    inference_validation_lifecycle --lib`; `cargo test -p
    pantograph-workflow-service
  ...
  - Discovered issue: `cargo check -p pantograph-workflow-service` still
    reports the pre-existing `set_active_run_execution_plan` dead-code warning.
- 2026-05-28 Milestone 5d validation publisher cancellation consumption slice:
  - Smallest useful vertical slice: consume lifecycle cancellation in the
    shared validation publisher after fact lookup and stop before publication
    acceptance or current-state recording when a validation session was
  ...
  - Allowed files touched:
    ...
    `crates/pantograph-workflow-service/src/graph/README.md`,
    the Milestone 5d file, and this execution log.
  - No-fallback/no-legacy result: cancellation is enforced through the
    workflow-service lifecycle owner and shared publisher, with no frontend
    filtering, transport retry path, legacy validator, or metadata inference.
  - Verification passed: `cargo fmt -p pantograph-workflow-service --
    --check`; `cargo test -p pantograph-workflow-service
    inference_validation_lifecycle --lib`; `cargo test -p
    pantograph-workflow-service
...
- 2026-05-28 Milestone 5d graph revision validation cancellation slice:
  ...
  - Discovered issue: `cargo check -p pantograph-workflow-service` still
    reports the pre-existing `set_active_run_execution_plan` dead-code warning.
- 2026-05-28 Milestone 5d node-data validation cancellation slice:
  ...
  - Allowed files touched:
    ...
    `crates/pantograph-workflow-service/src/graph/session_tests.rs`,
    the Milestone 5d file, and this execution log.
  - No-fallback/no-legacy result: inference-node data changes cancel active
    validation through the canonical lifecycle owner, with no frontend
    invalidation fallback or parallel stale-output publication path.
  - Verification passed: `cargo fmt -p pantograph-workflow-service --
    --check`; `cargo test -p pantograph-workflow-service
