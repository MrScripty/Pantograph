...
- Editor lifecycle projection consumption reads the backend current validation
  projection read model instead of calling refresh from lifecycle events. The
  read model returns stored backend summary plus node projections for the
  current graph revision, and returns typed stale/missing diagnostics with no
  projections when current state is unavailable.
