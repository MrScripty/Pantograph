...
- [x] Run affected Rust tests.
- [x] Run affected frontend tests and type checks.
- [x] Build frontend.
- [x] Build release binary.
- [x] Run serialization/contract tests for Rust DTOs and frontend TypeScript
  consumers touched by stale graph inspection or image-generation planning.
- [x] Run standards spot checks: no production `unwrap()`/`expect()` in new
...
- [x] Run lifecycle/transport spot checks for touched backend services:
...
- [x] Run boundary spot checks: saved workflow JSON, stale graph diagnostics,
...
- [x] Run accessibility spot checks for IO inspector graph/details controls and
  any new settings navigation.
- [x] Run path/resource spot checks for Pumas package roots, artifact roots,
...
- [x] Run default, no-default-features, and all-features checks for affected
  public crates when runtime feature flags or optional dependencies changed.
- [x] Run dependency ownership checks for any new Rust crate, Python package,
...
- [x] Run cross-platform spot checks for touched runtime/device code: platform
...
- [x] Run device/runtime spot checks for explicit CPU and CUDA requests on the
