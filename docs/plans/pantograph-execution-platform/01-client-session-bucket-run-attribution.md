...
### 2026-04-24 Workflow Run Boundary Hardening

- Updated the public generic `WorkflowService::workflow_run` boundary to reject
  caller-supplied `run_id` values. Backend-owned run ids remain generated by
  the service for non-attributed runs and are injected by the attributed
  workflow-run path after durable `WorkflowRunRecord` creation.
- Updated workflow-service unit and contract tests so generic workflow-run
  callers no longer assert caller-authored run ids as supported output.
- Verification: `cargo fmt --all -- --check`,
