...
| `13` | In progress | Neutral conversion contracts, command planning, workflow-service conversion hook, descriptor/diagnostics attribution metadata, managed executable resolution, Tauri host conversion adapter injection, cancellation/drop lease cleanup, descriptor metadata retention proof, managed executable fixture coverage, removal-race coverage, and frontend conversion metadata visibility are implemented. Remaining work: private temp-file fallback, real bundled-converter fixture tests, pre-descriptor conversion failure projection, and stage-end review. |
