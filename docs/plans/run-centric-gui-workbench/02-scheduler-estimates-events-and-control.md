...
actor scope. The first GUI-admin queue boundaries can cancel or reprioritize a
queued run by run id across sessions, leave the scheduler store as the
authority, and emit `gui_admin` queue-control events for accepted/correlated
denied decisions. Clone/resubmit, running-run cancellation, privileged
cross-session reorder, pause/resume, and other admin-scope event emitters
remain pending.
