        ...
        {#each agentState.activityLog as item (item.id)}
          <div transition:slide>
            ...
            {:else if item.type === 'tool_call'}
              ...
              <button
                on:click={() => toggleExpanded(item.id)}
                class="w-full text-left rounded-lg p-3 bg-amber-900/20 border border-amber-800/50 hover:bg-amber-900/30 transition-colors"
              >
                <div class="flex items-center gap-2 text-xs text-amber-400 mb-1">
                  ...
                  <span class="font-mono text-amber-300">{item.metadata?.toolName}</span>
                  <svg class="w-3 h-3 ml-auto transform {expandedItems.has(item.id) ? 'rotate-180' : ''}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                  </svg>
                </div>
