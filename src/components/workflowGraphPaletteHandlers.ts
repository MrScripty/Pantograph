...
function formatPaletteDropError(error: unknown): string {
...
function warnIfAddNodeDidNotApply(result: unknown, onFailure?: (message: string) => void): void {
  ...
  const errorDetail =
    'error' in result && result.error ? ` ${formatPaletteDropError(result.error)}` : '';
  const sessionDetail =
    'sessionId' in result && typeof result.sessionId === 'string'
      ? ` Session: ${result.sessionId}.`
      : '';
  const message =
    `Palette drop did not add a node: mutation ${String(status)}.${errorDetail}${sessionDetail}`;
