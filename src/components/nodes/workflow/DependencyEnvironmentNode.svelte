<script lang="ts">
  ...
  interface DependencyOverrideFieldsV1 {
    python_executable?: string;
    index_url?: string;
  ...
  interface DependencyOverridePatchV1 {
    contract_version: number;
    binding_id: string;
  ...
  interface DependencyActivityEvent {
    timestamp: string;
    node_type: string;
    model_path: string;
    phase: string;
  ...
  function normalizeOverridePatch(raw: unknown): DependencyOverridePatchV1 | null {
    ...
    if (!Number.isFinite(contract_version) || binding_id.length === 0 || scope.length === 0 || !rawFields) {
      return null;
    }
  ...
  function parseOverridePatches(raw: unknown): DependencyOverridePatchV1[] {
  ...
  function isPatchTarget(
    ...
    if (scope === 'requirement') {
      return (patch.requirement_name ?? '').trim().toLowerCase() === (requirementName ?? '').trim().toLowerCase();
    }
    return true;
  }

  function hasOverrideFields(fields: DependencyOverrideFieldsV1): boolean {
    return (
