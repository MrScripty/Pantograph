<script lang="ts">
  ...
  let isModelConnected = $derived(
    $edges.some((edge) => edge.target === id && edge.targetHandle === 'model')
  );

  // Processing node color (green)
  const nodeColor = '#22c55e';
...
<div class="ollama-node-wrapper" style="--node-color: {nodeColor}">
  <BaseNode {id} {data} {selected}>
    {#snippet header()}
      <div class="flex items-center gap-2">
        ...
        <span class="text-sm font-medium text-neutral-200">{data.label || 'Ollama Inference'}</span>
      </div>
    {/snippet}
...
<style>
  .ollama-node-wrapper :global(.base-node) {
    border-color: color-mix(in srgb, var(--node-color) 50%, transparent);
  }

  .ollama-node-wrapper :global(.node-header) {
