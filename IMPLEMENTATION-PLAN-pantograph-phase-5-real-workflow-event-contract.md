...
| -------- | --------------------------------- | ----------------------------------- | --------------------------------------- |
| `WaitingForInput` | `node-engine::DemandEngine::demand` emits `WorkflowEvent::WaitingForInput` for unresolved `human-input` nodes before returning `NodeEngineError::WaitingForInput`. | Tauri translates and traces it directly; the streamed Tauri edit-session execution runtime now also pins waiting-as-non-error transport semantics; `packages/svelte-graph` now reduces it through the store helper; UniFFI buffers preserve the canonical event name, its frontend-HTTP workflow-run/session-run bindings pin invalid-request parity, and its direct embedded-runtime `workflow_run`/`workflow_run_session` bindings now pin the same backend-owned interactive envelope; Rustler forwards the raw serialized engine event and now also pins the same session-run invalid-request contract on its frontend-HTTP host path; and the embedded runtime has focused non-streaming coverage for both direct `workflow_run` and session-backed `run_workflow_session` interactive mismatch envelopes. | Non-streaming embedded/frontend-http workflow runners still surface interactive mismatch through final error envelopes rather than a streamed event contract. Keep that behavior explicit until a dedicated headless streaming boundary exists. |
...
| `Cancelled` | `node-engine::WorkflowEvent` now includes canonical `WorkflowCancelled`, and backend Rust emitters can publish explicit cancellation semantics without encoding them as failure strings. Non-streaming workflow-service surfaces now also expose a first-class `cancelled` error envelope instead of flattening user-driven cancellation into `runtime_timeout`. | Tauri translation and diagnostics now consume `WorkflowCancelled` directly without classifying `WorkflowFailed` messages; UniFFI buffered events preserve the canonical cancellation type name and now also pin frontend-HTTP session-run cancelled-envelope parity; Rustler continues forwarding raw backend event JSON and therefore inherits the backend-owned variant directly, and its frontend-HTTP session host path now also pins the same cancelled contract; frontend HTTP mapping preserves the backend-owned `cancelled` error code instead of rewriting it into timeout semantics; the real frontend-HTTP `workflow_run` entrypoint has focused contract coverage for that envelope; and the concrete embedded-runtime `WorkflowHost::run_workflow` path now also has a focused pre-cancelled `WorkflowRunHandle` test at the host boundary. | Broaden canonical cancellation emission to any remaining backend-owned producer paths that can still terminate without publishing `WorkflowCancelled`, and extend acceptance coverage only where a remaining real non-streaming command or runtime-hosted binding surface is still not pinned. |
...
- 2026-04-18: The concrete embedded-runtime workflow host now also has focused
  contract coverage for a pre-cancelled `WorkflowRunHandle`. That keeps the
  backend-owned non-streaming cancellation contract pinned at the actual
  runtime host boundary instead of only through workflow-service, adapter, or
  binding wrappers.
- 2026-04-18: The embedded runtime session-backed non-streaming run path now
  also has focused interactive mismatch coverage. `EmbeddedRuntime::run_workflow_session`
...
- 2026-04-18: Rustler frontend-HTTP session-host coverage now also pins the
  ...
  envelope assertions.
- 2026-04-18: UniFFI direct embedded-runtime coverage now also pins the same
  backend-owned interactive `invalid_request` envelope on
  `FfiPantographRuntime::workflow_run` and
  `FfiPantographRuntime::workflow_run_session`. The real direct runtime
  binding path therefore no longer relies on frontend-HTTP parity coverage to
  prove the non-streaming interactive mismatch contract.
